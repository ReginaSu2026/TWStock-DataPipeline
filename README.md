...
本專案建置一個自動化的 **Data Pipeline**，定時擷取台灣股市之每日個股價量資訊與三大法人（外資、投信、自營商）籌碼數據。

資料處理後自動結構化寫入自建之 **MariaDB 資料庫**，確保數據的完整性與非重複性（Constraint Validation），提供後續量化策略回測（如：投信連買 + 布林通道突破）與 Power BI 視覺化分析之基礎。
...
| **資料庫** | `MariaDB`, `SQLAlchemy`, `PyMySQL` | 關係型資料庫儲存與 ORM 操作 |
| **版本控制** | `Git`, `GitHub` | CI/CD 流程與專案版本管控 |

---

## 🗄️ Database Schema 規劃

資料庫設計包含兩張核心資料表，並透過複合主鍵 `(stock_id, date)` 避免重複寫入：
...
| 欄位名稱 | 型別 | 說明 |
| :--- | :--- | :--- |
| `stock_id` **(PK)** | `VARCHAR(10)` | 股票代碼 (如: 2330) |
| `date` **(PK)** | `DATE` | 交易日期 (YYYY-MM-DD) |
| `open` / `high` / `low` / `close` | `DECIMAL(8,2)` | 開高低收價格 |
| `volume` | `BIGINT` | 成交股數 |
...
| 欄位名稱 | 型別 | 說明 |
| :--- | :--- | :--- |
| `stock_id` **(PK)** | `VARCHAR(10)` | 股票代碼 |
| `date` **(PK)** | `DATE` | 交易日期 |
| `foreign_buy` | `BIGINT` | 外資買賣超股數 |
| `investment_buy` | `BIGINT` | 投信買賣超股數 |
| `dealer_buy` | `BIGINT` | 自營商買賣超股數 |
